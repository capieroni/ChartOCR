...
std::vector<at::Tensor> pool_backward(at::Tensor input, at::Tensor grad_output) {
    at::Tensor output = at::zeros_like(input);
    ...
    auto max_val = at::zeros({batch, channel, height}, at::TensorOptions().dtype(at::kFloat).device(at::kCUDA));
    auto max_ind = at::zeros({batch, channel, height}, at::TensorOptions().dtype(at::kLong).device(at::kCUDA));
    ...
    auto gt_mask = at::zeros({batch, channel, height}, at::TensorOptions().dtype(at::kByte).device(at::kCUDA));
    auto max_temp = at::zeros({batch, channel, height}, at::TensorOptions().dtype(at::kFloat).device(at::kCUDA));
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    m.def("forward", &pool_forward, "Right Pool Forward", py::call_guard<py::gil_scoped_release>());
    m.def("backward", &pool_backward, "Right Pool Backward", py::call_guard<py::gil_scoped_release>());
}
